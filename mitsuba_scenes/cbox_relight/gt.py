...
def run_mitsuba(min_elev, max_elev, min_azim, max_azim, radius):
  for kind in kinds:
    for i, elev in enumerate(np.linspace(min_elev, max_elev, 4)):
      for j, azim in enumerate(np.linspace(min_azim, max_azim, 4)):
        ox, oy, oz = elaz_to_xyz(elev, azim, radius)
        for k, e2 in enumerate(np.linspace(min_elev, max_elev, 3)):
          for l, a2 in enumerate(np.linspace(min_azim, max_azim, 3)):
            lx, ly, lz = elaz_to_xyz(e2, a2, 1.05 * radius)
            scene = get_scene(ox, oy, oz, lx, ly, lz, kind)
            ...
            save_image(f"gt_{kind}_{i:03}_{j:03}_{k:03}_{l:03}.png", out)
