    ...
    for (int yi = 0; yi < S; ++yi) {
      ...
      for (int xi = 0; xi < S; ++xi) {
        ...
        for (int p = point_start_idx; p < point_stop_idx; ++p) {
          ...
          const float radius = points_a[p][3];
...
torch::Tensor RasterizeSpheresBackwardCpu(
  ...
  if (H != W) AT_ERROR("RasterizeSpheresBackwardCpu only supports square images");
  torch::Tensor grad_points = torch::zeros({P, 4}, points.options());
